...
if(${CURRENT_OS} STREQUAL "ios" OR ${CURRENT_OS} STREQUAL "android") 	
	list(APPEND RESOURCE_FILES "${CMAKE_CURRENT_SOURCE_DIR}/models/vnn_face278_data/face_mobile[1.0.0].vnnmodel")
else()
	list(APPEND RESOURCE_FILES "${CMAKE_CURRENT_SOURCE_DIR}/models/vnn_face278_data/face_pc[1.0.0].vnnmodel")
endif()


# Add platform source and header and lib link search path
IF(${CURRENT_OS} STREQUAL "android")							# android 
	LINK_DIRECTORIES(${CMAKE_CURRENT_SOURCE_DIR}/libs/${CURRENT_OS}/${ANDROID_ABI})
ENDIF()
...
ELSEIF(${CURRENT_OS} STREQUAL "windows")
	...
	${CMAKE_CURRENT_SOURCE_DIR}/libs/${CURRENT_OS}/x64/vnn_kit.lib)

	add_library(vnn_core SHARED IMPORTED)
	# 设置目标库的实际路径
	set_target_properties(vnn_core PROPERTIES IMPORTED_IMPLIB
	${CMAKE_CURRENT_SOURCE_DIR}/libs/${CURRENT_OS}/x64/vnn_core.lib)

	add_library(vnn_face SHARED IMPORTED)
	# 设置目标库的实际路径
	set_target_properties(vnn_face PROPERTIES IMPORTED_IMPLIB
	${CMAKE_CURRENT_SOURCE_DIR}/libs/${CURRENT_OS}/x64/vnn_face.lib)
ELSEIF(${CURRENT_OS} STREQUAL "macos" OR ${CURRENT_OS} STREQUAL "ios")
	set_target_properties(${PROJECT_NAME} PROPERTIES
		...
		LINK_FLAGS "-Wl,-F${CMAKE_CURRENT_SOURCE_DIR}/libs/${CURRENT_OS}"
	)
ELSEIF(${CURRENT_OS} STREQUAL "android")
	...
	# 设置目标库的实际路径
	set_target_properties(vnn_kit PROPERTIES IMPORTED_LOCATION
	${CMAKE_CURRENT_SOURCE_DIR}/libs/${CURRENT_OS}/${ANDROID_ABI}/libvnn_kit.so)

	add_library(vnn_core SHARED IMPORTED)
	# 设置目标库的实际路径
	set_target_properties(vnn_core PROPERTIES IMPORTED_LOCATION
	${CMAKE_CURRENT_SOURCE_DIR}/libs/${CURRENT_OS}/${ANDROID_ABI}/libvnn_core.so)

	add_library(vnn_face SHARED IMPORTED)
	# 设置目标库的实际路径
	set_target_properties(vnn_face PROPERTIES IMPORTED_LOCATION
	${CMAKE_CURRENT_SOURCE_DIR}/libs/${CURRENT_OS}/${ANDROID_ABI}/libvnn_face.so)
...
MACRO(EXPORT_INCLUDE)
	...
	SET(VNN_LIBS ${CMAKE_CURRENT_SOURCE_DIR}/libs/${CURRENT_OS})
